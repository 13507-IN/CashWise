    ...
    if (isset($_POST['quick_save']) && isset($_POST['ajax']) && $_POST['ajax'] === 'true') {
        ...
        if ($result->num_rows == 1) {
            ...
            if ($new_amount >= $goal_data['target_amount'] && $goal_data['current_amount'] < $goal_data['target_amount']) {
                ...
                $target_reached = true;
            } else if ($new_amount >= $goal_data['target_amount']) {
                $target_reached = true;
            }
            ...
            if ($stmt->execute()) {
                ...
                $response['newAmount'] = $new_amount;
                $response['targetAmount'] = $goal_data['target_amount'];
                
                // Set different messages based on goal status
                if ($just_reached_target) {
                    $response['message'] = 'Congratulations! You\'ve reached your goal!';
                    $response['goalReached'] = true;
                    $response['goalName'] = $goal_data['name'];
                } else if ($target_reached) {
                    $response['message'] = 'Amount saved successfully! Your goal is already complete.';
                    $response['goalReached'] = true;
                } else {
                    $response['message'] = 'Amount saved successfully!';
                    $response['goalReached'] = false;
                }
                
                // Also log this as a transaction if needed
                $category_id = 0; // You might need to get a savings category ID
                
                // You can add code here to log this as a transaction if needed
            } else {
